...
<?php endforeach; ?>
    ...
    public function actionCreate()
    {
        ...
        if ($this->request->isPost) {
            if ($model->loadAll($this->request->post()<?= !empty($generator->skippedRelations) ? ", [".implode(", ", $skippedRelations)."]" : ""; ?>) && $model->saveAll(<?= !empty($generator->skippedRelations) ? "[".implode(", ", $skippedRelations)."]" : ""; ?>)) {
...
<?php endif; ?>
    ...
    public function actionDelete(<?= $actionParams ?>)
    {
        ...
        $result = $this->findModel(<?= $actionParams ?>)->delete();
...
?>
        if (($model = <?= $modelClass ?>::findOne(<?= $condition ?>)) !== null) {
            ...
            throw new NotFoundHttpException(<?= $generator->generateString('Data tidak ditemukan.')?>);
        }
    }
<?php foreach ($relations as $name => $rel): ?>
<?php if ($rel[2] && isset($rel[3]) && !in_array($name, $generator->skippedRelations)): ?>
