...
class PegawaiController extends Controller
{
    ...
    public function actionView($id)
    {
        ...
        $providerPresensiPegawaiLog = new \yii\data\ArrayDataProvider([
            'allModels' => $model->presensiPegawaiLogs,
        ]);
        return $this->render('view', [
            'model' => $model,
            'providerCutiPegawaiData' => $providerCutiPegawaiData,
            'providerIzinPegawaiData' => $providerIzinPegawaiData,
            'providerPresensiPegawaiLog' => $providerPresensiPegawaiLog,
        ]);
    }
    ...
    */
    ...
    /**
    * Action to load a tabular form grid
    * for PresensiPegawaiLog
    * @author Yohanes Candrajaya <moo.tensai@gmail.com>
    * @author Jiwantoro Ndaru <jiwanndaru@gmail.com>
    *
    * @return mixed
    */
    public function actionAddPresensiPegawaiLog()
    {
        if ($this->request->isAjax) {
            $row = $this->request->post('PresensiPegawaiLog');
            if (!empty($row)) {
                $row = array_values($row);
            }
            if(($this->request->post('isNewRecord') && $this->request->post('_action') == 'load' && empty($row)) || $this->request->post('_action') == 'add')
                $row[] = [];
            return $this->renderAjax('_formPresensiPegawaiLog', ['row' => $row]);
        } else {
            throw new NotFoundHttpException('The requested page does not exist.');
        }
    }
