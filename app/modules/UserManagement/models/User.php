...
use app\modules\UserManagement\components\UserIdentity;
use app\modules\UserManagement\helpers\LittleBigHelper;
use app\modules\UserManagement\helpers\Singleton;
...
class User extends UserIdentity
{
	 ...
	 * @return bool
	 */
	public static function hasRole($roles, $superAdminAllowed = false)
	{
		if ($superAdminAllowed and (Yii::$app->user->isSuperadmin ?? false)) {
			return true;
		}
		$roles = (array)$roles;
		...
		if ($insert) {
			...
			if (php_sapi_name() != 'cli') {
				if (Yii::$app->user->id == $this->id) {
					...
					if ((Yii::$app->user->isSuperadmin ?? false) and $this->superadmin != 1) {
						$this->superadmin = 1;
					}
				}

				// Don't let non-superadmin edit superadmin
				if (isset($this->oldAttributes['superadmin']) && !(Yii::$app->user->isSuperadmin ?? false) && $this->oldAttributes['superadmin'] == 1) {
					return false;
				}
			}
		}
