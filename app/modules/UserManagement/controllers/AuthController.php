...
use app\modules\UserManagement\components\basecomponents\BaseController;
use app\modules\UserManagement\components\UserAuthEvent;
use app\modules\UserManagement\models\forms\ChangeOwnPasswordForm;
use app\modules\UserManagement\models\forms\ConfirmEmailForm;
use app\modules\UserManagement\models\forms\LoginForm;
use app\modules\UserManagement\models\forms\PasswordRecoveryForm;
...
class AuthController extends BaseController
{
	 ...
	 * @var array
	 */
	public $freeAccessActions = ['login', 'logout', 'confirm-registration-email', 'exit-login-as'];
	...
	public function actionLoginAs($id)
	{
		...
		Yii::$app->session->set('user.olduser', Yii::$app->user->identity->username);
		Yii::$app->user->login($model->getUser(), 0);

		return $this->redirect(['/site/index']);
	}

	public function actionExitLoginAs()
	{
		$model = new LoginForm();
		$model->username = Yii::$app->session->get('user.olduser');

		Yii::$app->user->login($model->getUser(), 0);
		Yii::$app->session->remove('user.olduser');

		return $this->redirect(['/user-management/user/index']);
	}
}
